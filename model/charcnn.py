...
class CharCNN(nn.Module):
    def __init__(self, alphabet_size, pretrain_char_embedding, embedding_dim, hidden_dim, dropout, gpu):
        ...
        if torch.cuda.is_available():
            self.char_drop = self.char_drop.cuda(gpu)
            self.char_embeddings = self.char_embeddings.cuda(gpu)
            self.char_cnn = self.char_cnn.cuda(gpu)
    ...
    def get_all_hiddens(self, input):
        """
            input:
                input: Variable(batch_size,  word_length)
                seq_lengths: numpy array (batch_size,  1)
    ...
    def forward(self, input):
        return self.get_all_hiddens(input)
