...
class AttenCNN_SentATTEN(nn.Module):
	def __init__(self, vocab, num_classes, char_alphabet):
		super(AttenCNN_SentATTEN,self).__init__()
		...
		#sentence atten
		self.atten_W = nn.Linear(self.embed_size, 1, bias=False)
		self.sent_hidden_size = opt.sent_hidden_size
		self.sent_hidden = nn.Linear(self.embed_size, self.sent_hidden_size)
	...
	def forward(self, input, char_inputs,sent_inputs):
		...
		flat_input = sent_embedding.contiguous().view(-1, self.embed_size)
