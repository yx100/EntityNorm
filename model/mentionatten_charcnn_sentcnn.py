...
class AttenCNN_SentCNN(nn.Module):
	def __init__(self, vocab, num_classes, char_alphabet):
		super(AttenCNN_SentCNN,self).__init__()
		...
		self.sent_convs1 = nn.ModuleList([nn.Conv2d(in_channels=Ci, out_channels=Co, kernel_size=(K, self.embed_size), stride=(1, 1),
	...
	def forward(self, input, char_inputs,sent_inputs):
		...
		sent_feature = [functional.relu(conv(sent_feature)).squeeze(3) for conv in self.sent_convs1]  # [(N, Co, W), ...]*len(Ks)
		sent_feature = [functional.max_pool1d(i, i.size(2)).squeeze(2) for i in sent_feature]  # [(N, Co), ...]*len(Ks)
